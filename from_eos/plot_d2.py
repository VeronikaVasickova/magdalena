...
for key in df.columns:
    if "SEY_" in key and "_D2B1" in key and ("13L5" not in key):
        print(key)
        d2_keys.append(key)

d22_keys = []
for key in df.columns:
    if "SEY_" in key and "_D2B2" in key:
        print(key)
        d22_keys.append(key)
...
for key in d2_keys:
    ax1.plot(df.index, df[key],'-o', label=f"D2B1_{key[10:14]}", ms=ms_size)
...
ax1.set_title("Instrumented cells D2, Beam 1")
ax1.xaxis.set_major_locator(plt.MaxNLocator(10))
fig1.subplots_adjust(left=.1, right=.76, hspace=.28, top=.95)
fig1.savefig(fig_folder+"instr_d2_b1.png")


fig2 = plt.figure(2, figsize=figure_size)
ax2 = fig2.add_subplot(111)
for key in d22_keys:
    ax2.plot(df.index, df[key],'-o', label=f"D2B2_{key[10:14]}", ms=ms_size)
...
ax2.set_title("Instrumented cells D2, Beam 2")
ax2.xaxis.set_major_locator(plt.MaxNLocator(10))
fig2.subplots_adjust(left=.1, right=.76, hspace=.28, top=.95)
fig2.savefig(fig_folder+"instr_d2_b2.png")

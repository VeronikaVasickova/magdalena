...
df = pd.DataFrame.from_dict(json.load(open(result_folder + "arcs_df.json","r")))
df.index = df.index.map(int)
df = df.sort_index()

a_keys = []
for key in df.columns:
    if "SEY_" in key and "AVG_ARC" in key:
        print(key)
        a_keys.append(key)
...
for ii,key in enumerate(a_keys):
    colorcurr = ms.colorprog(i_prog=ii, Nplots=8)
    ax1.plot(df.index, df[key],'o', label=f"{key[4:7]}", ms=ms_size, color=colorcurr)
...
fig1.savefig(fig_folder+"arcs.png")

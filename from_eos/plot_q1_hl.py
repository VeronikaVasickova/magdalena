...
for pair in fills_times.get_fill_time_pairs_one_time():
    df1 = pd.DataFrame.from_dict(json.load(open(data_folder + f"meas_hl_Fill{pair[0]}_T{pair[1]:.2f}.json","r")))[['special_Q1B1']].dropna(how='all')
    df1 = df1.T
    df1['Fill'] = pair[0]
    df1 = df1.set_index('Fill')
    
    df2 = pd.DataFrame.from_dict(json.load(open(data_folder + f"meas_hl_Fill{pair[0]}_T{pair[1]:.2f}.json","r")))[['special_Q1B2']].dropna(how='all')
    df2 = df2.T
    df2['Fill'] = pair[0]
...
for col in q1.columns:
    ax1.plot(q1.index, q1[col],'-o', ms=ms_size, label=f"Q1B1_{col[6:10]}")
...
ax1.set_title("Heatloads in Q1, Beam 1")
ax1.xaxis.set_major_locator(plt.MaxNLocator(10))
fig1.subplots_adjust(left=.1, right=.76, hspace=.28, top=.95)
fig1.savefig(fig_folder+"hl_instr_q1_b1.png")
...
for col in q2.columns:
    if "13L5" in col:
        continue
    ax2.plot(q2.index, q2[col],'-o', label=f"Q1B2_{col[6:10]}", ms=ms_size)
...
ax2.set_title("Heatloads in Q1, Beam 2")
ax2.xaxis.set_major_locator(plt.MaxNLocator(10))
fig2.subplots_adjust(left=.1, right=.76, hspace=.28, top=.95)
fig2.savefig(fig_folder+"hl_instr_q1_b2.png")
